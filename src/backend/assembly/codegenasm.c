...
char* codegenasm_get_operand_name(CodeGenerator* generator, IROperand* operand) {
    ...
    switch (operand->type) {
        case IR_OP_TEMP:
            return codegenasm_get_temp_name(generator, operand);
        case IR_OP_VAR:
            return operand->data.var_name;
        case IR_OP_CONST:
            return codegenasm_get_const_name(generator, operand);
        case IR_OP_LABEL:
            return operand->data.label_name;
        default:
            return "0";
    }
}
...
char* codegenasm_get_const_name(CodeGenerator* generator, IROperand* operand) {
    (void)generator; 
    char* const_name = safe_malloc(32);
    snprintf(const_name, 32, "const_%lld", operand->data.const_value);
    return const_name;
}
